...
export function useRepository(): UseRepositoryReturn {
  ...
  const cloneAndIngest = useCallback(
    async (githubUrl: string): Promise<ApiResponse<RepoResponse>> => {
  ...
  const generateCodeTree = useCallback(
    async (githubUrl: string): Promise<ApiResponse<any>> => {
      setState(prev => ({ ...prev, isLoading: true, error: null }));
      ...
      try {
        ...
        setState(prev => ({
          ...
          error: result.success
            ? null
            : result.error || "Failed to generate code tree",
        }));
